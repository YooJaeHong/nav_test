...
public class github_api_parser extends AsyncTask<String, Void, String> {
    ...
    protected String doInBackground(String... params) {
        ...
        try {
            ...
            conn.setRequestProperty("Authorization","token 23a1f34ac96ed5e1c7bae06fc2895bad0508f426");
    ...
    protected void onPostExecute(String s) {
        String[][] url_list =  this.get_commit_URL(receiveMsg);

        for(int i=0;i<url_list.length;i++){
            for(int j=0;j<url_list[i].length;j++){
                github_api_parser commit_url_parser = new github_api_parser(url_list[i][j]);

                commit_url_parser.execute();
